...
public class HibernateSupport extends HibernateTemplate implements HibernateSupportTemplate {
    ...
    public List pageListBySql(org.pomo.commons.model.page.PageModel page,
        ...
        // Jerry tips：这句没看懂，又是setMaxResults的又是setFirstResult的
    ...
    public List<?> pageListBySql(PageModel page, String sqlCount, String sqlInfo, Class<?> clazz, Object... params) {
        ...
        // Jerry tips：QHHC的分页逻辑，通过mysql的limit实现
        // setMaxResults设置每页显示的记录数，比如每页显示20条记录
        // setFirstResult则设置第n页从(page.getPageNum() - 1) * page.getNumPerPage()开始，比如第1页从0开始，第2页从20开始，第3页从40开始
        // 上面的代码转换为mysql的分页逻辑：limit 20,20
...
//    }
    ...
    public ResultSet callProcedureResultSet(String procedureName, Object... params) {
    	...
    	ResultSet rs = getSession().doReturningWork(new ReturningWork<ResultSet>() {

			@Override
			public ResultSet execute(Connection connection) throws SQLException {
				// TODO Auto-generated method stub
				CallableStatement cs = connection.prepareCall(bornExecuteProcedureStr(procedureName, params));
				return installParams(cs, params).executeQuery();
			}
...
//    }
    ...
    public Object callProcedureOutput(String procedureName, Object... params) {
    	...
    	CallableStatement cs = getSession().doReturningWork(new ReturningWork<CallableStatement>() {

			@Override
			public CallableStatement execute(Connection connection) throws SQLException {
				// TODO Auto-generated method stub
				CallableStatement cs0 = connection.prepareCall(bornExecuteProcedureStr(procedureName, params));
				cs0.registerOutParameter(params.length, Integer.parseInt(params[params.length - 1].toString()));
				
				Object[] realParams = new Object[params.length - 1];
				for (int i = 0; i < realParams.length; i++) {
					realParams[i] = params[i];
				}
				installParams(cs0, realParams).execute();
...
//    }
    ...
    public boolean callProcedureVoid(String procedureName, Object... params) {
    	CallableStatement cs = getSession().doReturningWork(new ReturningWork<CallableStatement>() {
			...
			public CallableStatement execute(Connection connection) throws SQLException {
				// TODO Auto-generated method stub
				return connection.prepareCall(bornExecuteProcedureStr(procedureName, params));
			}
        	
        });
    	
    	try {
			return !installParams(cs, params).execute();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
