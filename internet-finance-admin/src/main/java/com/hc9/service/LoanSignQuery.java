...
//	}
	...
	public List<LoansignType> queryLoanType() {
		List<LoansignType> list = (List<LoansignType>) dao.find("from LoansignType");
		return list;
	}
	...
	public Loansign getYouxuan() {
		...
		List<Loansign> list = (List<Loansign>) dao.find(sb.toString());
	...
	public Loansignbasics getLoansignbasicsById(String loanSignId) {
		...
		List<Loansignbasics> lsbList = (List<Loansignbasics>) dao.find(sb.toString());
	...
	public Repaymentrecord getRepaymentByLSId(String loanSignId) {
		...
		List<Repaymentrecord> rmList = (List<Repaymentrecord>) dao.find(sb.toString());
...
//		dao.flush();
	...
	private void copyMsgReminderList(String loanSignId, Long newId, HttpServletRequest request) {
		...
		List<MsgReminder> list = (List<MsgReminder>) dao.find(sql);
...
//			dao.saveOrUpdateAll(voList);
	...
	private void copyAttchmentList(String loanSignId, Loansign newLoansign) {
		...
		if(list != null && list.size() > 0) {
			...
			for (Attachment entity : resultList) {
				dao.save(entity);
			}
