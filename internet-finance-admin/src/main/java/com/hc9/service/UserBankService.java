...
public class UserBankService {
	...
	public UserBank getUserBank(Long userId) {
		...
		List<UserBank> list = (List<UserBank>) dao.find(hql, userId);
		if (list.size() > 0) {
			return list.get(0);
		}
		return null;
	...
	public List<Banktype> getBankType() {
		...
		List<Banktype> list = (List<Banktype>) dao.find(hql.toString());
		return list;
	}
	...
	public UserBank getUserBankById(Long id) {
		...
		List<UserBank> list = (List<UserBank>) dao.find(hql, id);
		if (list.size() > 0) {
			return list.get(0);
		}
		return null;
	...
	public Validcodeinfo codeUserId(Long id) {
		...
		List<Validcodeinfo> list = (List<Validcodeinfo>) dao.find(hql, id);
		if (list.size() > 0) {
			return list.get(0);
		}
		return null;
