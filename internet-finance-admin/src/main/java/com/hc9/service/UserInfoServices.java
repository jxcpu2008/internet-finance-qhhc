...
public class UserInfoServices {
	...
	public List queryUserPage(PageModel page, Userbasicsinfo userinfo) {
		...
		// Jerry tips：下面的sql语句拼接简直了，这样拼接出来的sql语句执行效率非常差，如果非要让hibernate执行sql语句，则按照queryUnnormalUserInfosByPage方法所示做sql语句的优化
		// 统计数据条数sql拼接
		...
		// 方法签名对应：public List<?> pageListBySql(PageModel page, String sqlCount, String sqlInfo, Class<?> clazz, Object... params)
		datalist = dao.pageListBySql(page, countsql.toString(), sqlbuffer.toString(), null);

		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("queryUserPage(PageModel, Userbasicsinfo)方法结束OUTPARAM=" + datalist);
		}
		return datalist;
	...
	public Userrelationinfo queryRelationById(String ids) {
		...
		if (StringUtil.isNotBlank(ids) && StringUtil.isNumberString(ids)) {
			...
			relationList = (List<Userrelationinfo>) dao.find(hql);
