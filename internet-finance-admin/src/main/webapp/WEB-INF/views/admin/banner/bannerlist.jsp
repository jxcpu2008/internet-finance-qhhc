...
<div class="panelBar">
		<ul class="toolBar">
			...
			<li><a class="add" href="/banner/duplicateAppBannerFromH5?id={sid_obj}"  target="ajaxTodo" title="确定要复制为APP的banner吗？" warn="请选择H5的banner"><span>添加APP banner</span></a></li>
		</ul>
</div>
...
<table class="table" width="100%" layoutH="70" height="100%" rel="jbsxBox2">
	...
	<c:forEach items="${list}" var="banner">
		<tr target="sid_obj" rel="${banner[0]}" class="checkall" onclick="javascript:selecteddefault(${banner[0]});" style="cursor: pointer;" ondblclick="javascript:dbltr(${banner[0]});" >
			...
			<td>${banner[2]}</td>
			<td>${banner[3]}</td>
			<td>${banner[4] eq 1 ? 'PC端':(banner[4] eq 3 ? 'H5':'APP')}</td>
		</tr>
	</c:forEach>
	</tbody>
</table>
