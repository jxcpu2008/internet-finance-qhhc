...
<div class="pageContent">
	<form method="post" action="/banner/updatebanner" class="pageForm required-validate" enctype="multipart/form-data" onsubmit="return iframeCallback(this, dialogAjaxDone)">
		<div class="pageFormContent" layoutH="58">
			...
			<div class="unit">
				<label>所属位置：</label>
				<input type="radio" name="type" value="1" <c:if test="${banner.type eq 1 || empty banner.type}">checked="checked"</c:if>/>PC端
<%-- 				<input type="radio" name="type" value="2" <c:if test="${banner.type eq 2}">checked="checked"</c:if>/>我要投资 --%>
				<input type="radio" name="type" value="3" <c:if test="${banner.type eq 3}">checked="checked"</c:if>/>H5
			</div>
		</div>
		<div class="formBar">
			<ul>
