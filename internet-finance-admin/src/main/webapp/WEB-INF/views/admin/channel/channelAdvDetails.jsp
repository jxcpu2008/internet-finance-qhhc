...
<script type="text/javascript"
	src="/resources/assets/js/admin/channelAdvv.js"></script>
<div style="overflow-y: auto; height: 600px;">
	<input type="hidden" id="advId" value="${sId }" /> <input
		type="hidden" id="advcode" value="${code }" />
	<div style="padding: 40px 50px;">
		<button type="button" class="addAdvv" data-sid="${sId }"
			data-id="${code }" data-name="${name}"
			style="padding: 10px 35px; background-color: #0099FF; color: #fff; border-style: none; cursor: pointer;">新增广告</button>
	</div>
	<div
		...
		<ul style="overflow: hidden;">
			<c:forEach items="${page.list }" var="advList">
				<li
					...
					style="font-size: 14px;">${advList[2] }</span>
					<hr
						style="width: 80%; margin: -15px auto auto; color: #efefef; line-height: 25px;" />
					<button type="button" class="advv_edit" data-id="${advList[1]}"
					...
					<button type="button" class="advv_code" data-id="${advList[1]}"
						data-sid="${advList[0]}"
	...
	<jsp:include page="/WEB-INF/views/admin/channel/pageRedthree.jsp"></jsp:include>
</div>
<!-- ----弹出框 -->
<div id="domelChannelAdvv"
