	...
	<ul class="list-unstyled list-inline page_ul">
		...
		<c:if test="${page.totalPage<8}">
			<li><a href="javascript:"
				onclick="<c:if test="${page.pageNum!=1}">jumpPage2(1,${page.totalPage})</c:if>"
				class="page_s first">首页</a></li>
			<li><a href="javascript:"
				onclick="jumpPage2(${page.pageNum-1 },${page.totalPage})"
				class="page_s prev">上一页</a></li>
			<c:forEach var="no" begin="1" end="${page.totalPage}" varStatus="s">
				<li><a href="javascript:"
					onclick="<c:if test="${page.pageNum!=no}">jumpPage2(${no})</c:if>"
		...
		<c:if test="${page.totalPage>=8}">
			...
			<c:forEach var="no" begin="${startNo}" end="${startNo+2}"
				varStatus="s">
				<li><a href="javascript:"
					onclick="<c:if test="${page.pageNum!=no}">jumpPage2(${no},${page.totalPage})</c:if>"
					class="<c:if test="${page.pageNum==no}">page_s_1</c:if><c:if test="${page.pageNum!=no}">page_s</c:if>">${no}</a></li>
			</c:forEach>
		...
		<li><a href="javascript:"
			onclick="<c:if test="${page.pageNum!= page.totalPage}">jumpPage2(${page.totalPage},${page.totalPage})</c:if>"
