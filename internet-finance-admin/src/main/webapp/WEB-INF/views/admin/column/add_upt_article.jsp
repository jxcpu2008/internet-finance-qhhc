...
<div class="pageContent">
	<form  id="form1"  action="/article/addOrUpdateArticle?operation=${operation}"  enctype="multipart/form-data"   method="post" class="pageForm required-validate" onsubmit="return iframeCallback(this, navTabAjaxDone)" >
		...
		<div class="pageFormContent" layoutH="56">
			...
			<div style="width:600px; margin:0 auto;">
				...
				<div style="width:30%; float: left; margin-top: 10px;"><p>
					<label>是否显示：</label>
				...
				<div style="width:40%; float: left; margin-top: 10px;"><p>
				...
				<div style="width:30%; float: left; margin-top: 10px;">
					<p>
						<label>是否推送：</label>
						<c:if test="${article.isPush == '1'}"><input type="checkbox" name="isPush" checked="checked" value="1"></c:if>
		   		        <c:if test="${article.isPush != '1'}"><input type="checkbox" name="isPush" value="1"></c:if>
					</p>
				</div>
				
			<!--  	<div style="width:30%; float: left; margin-top: 10px;"><p>
					<label>是否头条：</label>
					 <c:if test="${article.orderNum == '1'}"><input type="checkbox" readonly="readonly" name="orderNum" checked="checked" value="1"  id="orderNum"></c:if>
	   		 	     <c:if test="${article.orderNum != '1'}"><input type="checkbox" readonly="readonly" name="orderNum" value="1" ></c:if>
				</p></div>		
				  <div style="width:100%; float: left; margin-top: 10px;"  class="filePathId"><p>
					<label>图片上传：</label>
					<input type="file" name="fileurl" <c:if test="${empty article.filePath}">class="required"</c:if> value="${article.filePath}"/>
					<input type="hidden" name="filePath" value="${article.filePath}"/>
				</div>		
				-->
				<div style="width:100%; float: left; margin-top: 10px;"><p>
					<label>一级栏目：</label>
				...
				<div style="width:100%; float: left; margin-top: 10px;"><p>
					<label>二级栏目：</label>
					 <select id="deputysection" name="deyId" class="required " style="width: 135px;" <c:if test="${operation eq 'upt'}">disabled="disabled"</c:if> onchange="deputsectionChange();">
			    	   <option value="${article.deputysection.id}" selected="selected">${article.deputysection.name}</option>
			    	   </select>
				</p></div>
				<div style="width:100%; float: left; margin-top: 10px;"><p>
					<label>内容编辑器：</label>
					<textarea class="editor" name="context" rows="13" cols="80" id="context"
