...
<div class="pageContent">
	<form method="post" action="/loanSignOfCir/saveORupdateLoansignSic" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
		...
		<input type="text" style="display: none;" name="id" value="${loansign.id}"/>
			...
			<p>
				<label>借款标类型</label>
				<input name="loanCategory" value="1"  <c:if test="${loansignbasics.loanCategory ne  '2'}">checked="checked"</c:if> type="radio"/>富薪贷&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="loanCategory"  <c:if test="${loansignbasics.loanCategory eq  '2'}">checked="checked"</c:if> value="2" type="radio"/>富商贷
			</p>
