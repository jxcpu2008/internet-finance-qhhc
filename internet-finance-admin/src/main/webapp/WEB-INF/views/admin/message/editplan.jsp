...
<script>
				...
				success : function(data) {
					...
					if(code == '0') {
						$.pdialog.open('/messageManager/viewSmsEmailTemplateOfPlan?msgType=' + msgType + '&templateType=' + templateType + '&templateEnName=' + templateEnName,
								'viewSmsEmailTemplateOfPlan', '查看短信邮件模板详情', {
								width : 800,
								height : 420
							});
					} else {
	...
	function submitPlan(plantype) {
		...
		$.ajax({
			url : '/messageManager/doModifyPlanPage.htm?planType=' + plantype,
			type : 'post',
			...
			success : function(data) {
				...
				if ("0" == code) {
					...
					if (plantype == 1) {
						if (type == 1) {
							//navTab.closeTab();
							navTab.openTab("main103", "/messageManager/smsList?type=1", 
									{ title:"短信列表", fresh:true});
						} else {
							navTab.openTab("main102", "/messageManager/smsList?type=2", 
...
<div style="background-color: #fff; width: 100%; height: 400px; top: 100px; left: 400px;">
	...
	<form method="post" id="planform">
		<div class="conment" style="padding-top: 20px; padding-left: 50px;">
			<input type="hidden" name="planId" value="${smsemailSendPlan.id}"/>
			<input type="hidden" name="planTimeId" value="${smsEmailPlanTime.id}"/>
			<input type="hidden" name="createTime" value="${smsemailSendPlan.createTime}"/>
			<input type="hidden" id="userChangeFlag" name="userChangeFlag" value="0">
			<ul>
				<li>
					...
					<select style="width: 180px;" id="sendTypeId" name="sendType">
						...
						<option value="1"
							<c:if test="${smsemailSendPlan.sendType eq 1 }">selected="selected"</c:if>>短信</option>
						<option value="2"
							<c:if test="${smsemailSendPlan.sendType eq 2 }">selected="selected"</c:if>>邮件</option>
					</select>
				</li>
				<li>
					<label style="margin-right: 40px;">&nbsp;&nbsp;收件人:</label>
					<input type="text" style="width: 380px;" id="getUserPerson" value="${receiveName}" readonly="readonly" />
				...
				<li>
					...
					<select style="width: 190px;" id="emailType" name="templateType">
						...
						<c:forEach items="${oneLevelTypeList }" var="emailType">
							<option value="${emailType.switchEnName}" <c:if test="${emailType.switchEnName==smsemailSendPlan.templateType}">selected="selected"</c:if>>${emailType.switchZhName}</option>
						</c:forEach>
					</select>
					<select style="width: 190px;" id="emailName" name="templateEnName">
						<option value="">请选择</option>
						<c:forEach items="${twoLevelTypeList }" var="emailType">
							<option value="${emailType.switchEnName}" <c:if test="${emailType.switchEnName==smsemailSendPlan.templateEnName}">selected="selected"</c:if>>${emailType.switchZhName}</option>
						</c:forEach>
					</select>
				...
				<li>
					...
					class="date" dateFmt="yyyy-MM-dd HH" style="width: 180px" value="${smsEmailPlanTime.sendBeginTime}"
					placeholder="开始时间" /> - 
					<input id="end_send_time" type="text"
					name="sendEndTime" class="date" dateFmt="yyyy-MM-dd HH" value="${smsEmailPlanTime.sendEndTime}"
				...
				<li>
					...
					<input id="msgNum" type="text" onkeyup="value=value.replace(/[^\d]/g,'')" name="msgNum" value="${smsEmailPlanTime.msgNum}" style="width: 30px;" />
					封
				</li>
			</ul>
...
<div id="chooseGetUser" style="border: 1px solid #CCC; background-color: #fff; width: 650px; height: 440px; 
	...
	<form id="searchForm" method="post">
		<div class="conment">
			<ul>
				<li><label>&nbsp;投资总额：</label><input id="loanfloor"
				...
				<li><label>投资时间：</label> <input type="text"
					name="loanTimeFloor" id="loanTimeFloor" /> - <input type="text"
					name="loanTimeup" id="loanTimeup" /></li>
				<li><label>投资次数：</label><input id="loanCountFloor"
					onkeyup="value=value.replace(/[^\d]/g,'')" type="text"
					name="loanCountFloor" /> - <input type="text" id="loanCountUp"
					onkeyup="value=value.replace(/[^\d]/g,'')" name="loanCountUp" /></li>
				<li><label>注册时间：</label> <input type="text" name="regTimeFoolr"
					id="regTimeFoolr" /> - <input type="text" name="regTimeUp"
					id="regTimeUp" /></li>
				<li><label>&nbsp;活 跃 度：</label><input type="radio"
					name="liveness" value="1" /><font>近一周内登录过</font><input
					type="radio" name="liveness" value="2" /><font>近一周内没有登录过</font></li>
				<li style="margin-left: 62px;"><input type="radio"
					name="liveness" value="3" /><font>近一个月内登录过</font><input
					type="radio" name="liveness" value="4" /><font>近一个月内没有登录过</font></li>
		...
		<div style="border-top: 1px solid #ccc; padding: 20px; height: 30px;">
			<span id="result">根据筛选条件，共搜出符合条件的数据<span id='user_num'>${userNum}</span>条。其中${notEmailNum}个用户没有邮箱。</span> <span style="float: right;">
