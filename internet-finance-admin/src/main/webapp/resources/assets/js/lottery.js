...
var lottery = function() {
	...
	if (islogin == "") {
		ymPrompt.errorInfo("抱歉，您还未登陆哦！", 450, 240, "失败", function(result) {
			$("#zcModal_login").modal('show');
		});
		$('.ymPrompt_error').css('height', '140px');
		return;
	} else {
		ymPrompt.errorInfo("<p style='font-weight:700;padding-bottom:20px;'>抱歉，本次活动已结束<p><p style='font-size:14px;'>八月理财活动正在如火进行，<a href='http://www.hc9.com/august.htm' style='color:#d9534f;text-decoration:underline;'>前去围观</a></p>", 450, 240, "失败", function(result) {
			$("#myModal").modal('show');
		});
		$('.ymPrompt_error').css('height', '140px');
		return;
