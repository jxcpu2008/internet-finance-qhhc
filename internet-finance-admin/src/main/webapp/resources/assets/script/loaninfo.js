			...
			function() {
				...
				$.ajax({
					...
					success : function(data) {
						...
						if (result.code == 0) {
							$('body').removerSpinner();
							if (result.openCardFlag == 1) {   // 登顶活动翻牌抽奖活动情况下
								var num = 0;
								$.ajax({url : '/april/getOpenCardLotteryNum.htm',type : 'post',success : function(msg) {
									msg = JSON.parse(msg);
									num = msg.totalNum;
									if (num > 0) {
										ymPrompt.alert({message : "恭喜您，认购成功，您现在有"+num+"次抽奖机会！",width : 420,height : 200,title : "提示",handler : function(result) {
											if (result != "close") {
												window.location.href = '/april/toclimbtop.htm?sit=1';
											} else {
