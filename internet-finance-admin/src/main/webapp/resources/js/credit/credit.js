				...
				function() {
					...
					$("#myloanlist-button-search").click(
							function() {
								...
								ajaxbg.show();
								var url = encodeURI("/credit/creditSearch?"
										+ $("#myloanlistsearchfrom")
												.serialize());
					...
					$("#myloanlist-button-search").click(function() {
						if($("#typeChannel")==null || $("#typeChannel").val().length<1){
							alert("请选择渠道");
							return;
						}
						if($("#name")==null || $("#name").val().length<1){
							alert("请填写姓名");
							return;
						}
						if($("#issueLoanfind")==null || $("#issueLoanfind").val().length<1){
							alert("请选择证件类型");
							return;
						}
						if($("#idNo")==null || $("#idNo").val().length<1){
							alert("请填写证件号");
							return;
						}
						if($("#mobileNo")==null || $("#mobileNo").val().length<1){
							alert("请填写手机号码");
							return;
						}
						if($("#cardNo")==null || $("#cardNo").val().length<1){
							alert("请填写银行卡号");
							return;
						}
						if($("#recommendfind")==null || $("#recommendfind").val().length<1){
							alert("请选择查询原因");
							return;
						}
						
						jQuery.ajax({
							url:"/credit/creditSearch?",
							data:$('#myloanlistsearchfrom').serialize(),
							type:"POST",
							dataType:"JSON",
							success:function(data){
								if(data["dataStatus"] && data["dataStatus"] != null){
									//黑名单
									$("#blachList").text("是");
								}else{
									$("#blachList").text("否");
									$("#credooScore").text(data["credooScore"]);
									$("#finRequireScore").text(data["finRequireScore"]);
									$("#actionScore").text(data["actionScore"]);
									$("#payAbilityScore").text(data["payAbilityScore"]);
									$("#virAssetScore").text(data["virAssetScore"]);
									$("#performScore").text(data["performScore"]);
									$("#trendScore").text(data["trendScore"]);
								}
							}
						});
					});
