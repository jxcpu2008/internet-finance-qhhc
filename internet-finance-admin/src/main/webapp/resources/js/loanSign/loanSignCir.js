$(document).ready(function(){
	...
	/***************按钮及点击事件************/
		...
		$("#EXPloansignCIR").click(function(){
	        window.location.href='/loanSignOfCir/outPutExcel';
		});
            			...
            			$.ajax({
				    			...
				    			success:function(msg){
				    				if(msg==5){
				    					...
				    					alertMsg.info("放款失败,该项目未发布,请尝试刷新页面！");
				    				}else if(msg==1){
				    					alertMsg.info("放款失败,该标还在进行中,请尝试刷新页面！");
				    				}else if(msg==3){
				    					alertMsg.info("放款失败,该标在还款中,请尝试刷新页面！");
				    				}else if(msg==4){
				    					alertMsg.info("放款失败,该标已完成,不能在放款,请尝试刷新页面！");
				    				}else if(msg==6){
   ...
   // 数据源面板
		$('#mygridCIR1').omGrid({
			dataSource : '/loanSign/loanflowList',
			title : '项目众筹列表',
			height : 336,
			limit : 10,
			colModel : [ {
