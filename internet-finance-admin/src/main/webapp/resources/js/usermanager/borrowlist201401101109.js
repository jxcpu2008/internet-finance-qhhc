$(document).ready(function(){
	...
	$('#borrow_show_list').omGrid({
        ...
        colModel : [ {header : '序号', name : 'id', width : 50, align : 'center',editor:{editable:false}}, 
                     ...
                     {header : '信用等级分数', name : 'credit_rate', align : 'left', width : 80,editor:{editable:false}}, 
...
//	 });
	...
	//不通过
	 ...
	 //设置信用等级
	 $("#query-creditRate").click(function(){
		 //获取选中行数据
		 var selectedRecords = $('#borrow_show_list').omGrid('getSelections',true);
		 //判断用户是否选中数据
		 if(selectedRecords.length==1){
			 if(selectedRecords[0].ispass==2){
				 $.pdialog.open('/userinfo/queryByCreditRate?bid='+selectedRecords[0].id, 'update_creditRate', '设置信用等级' , { width:400,height:250,mask:true,maxable:false });
			 }else{
				 alertMsg.warn("只有审核通过的借款人才能设置信用等级！"); 
			 }
		 }else{
			alertMsg.warn("请选择您要设置的一个借款人"); 
		 }
