...
var ProcessCircle = function() {
    var processGroup = [];
    var products = $('#pro-stress').offset() ;					// 产品列表的距离文档顶部与左边偏移量
    products = typeof products === 'undefined' ? null : products;
    var productsHeight = $('#pro-stress').height();				// 产品高度
...
var Home = function() {
	var init = function() {
		...
		$('.icon-quns').mouseenter(function() {
			$(this).parents('ul').next().next().next('.ewm_quns').stop().slideDown('fast');
		});
		$(".ewm_quns").hover(function(){
			$(this).parents('ul').next().next().next('.ewm_quns').show();
		},function(){
