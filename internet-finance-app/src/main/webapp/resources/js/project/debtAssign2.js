function showAssign(loanId,loanTitile,tenderMoney,loanUnit){
	...
	content+="<div class=\"applyTransDiv\"><input type=\"hidden\" value=\""+tenderMoney+"\" id=\"tenderMoney\">";
	content+="当前申请转让的标为：<font style=\"color:red\">"+loanTitile+"（￥"+tenderMoney+"）</font>"
	...
	content+="	转让份额：";
	content+="    <select style='height: 30px;' name=\"transPercent\" id=\"transPercent\">";
	content+="        <option value=\"1\">全部转让</option>";
	content+="        <option value=\"2\">部分转让</option>";
	content+="    </select>";
	content+="    <input type=\"text\" name=\"transNum\" id=\"transNum\" class='vipTextBox' value=\""+parseInt(tenderMoney)/parseInt(loanUnit)+"\" width=\"80\" readonly=\"readonly\">份，共<span style=\"color:red\">"+tenderMoney+"</span>元（每份"+parseFloat(loanUnit).toFixed(1)+"元）";
	content+="</div>";*/
	/*content+="<div class=\"applyTransDiv\">";
	content+="	转让价格：";
	content+="    <select style='height: 30px;' name=\"transValue\" id=\"transValue\">";
	content+="        <option value=\"3\">不变</option>";
	content+="        <option value=\"1\">折扣</option>";
	content+="        <option value=\"2\">升值</option>";
	content+="    </select>";
	content+="    <input type=\"text\" name=\"percent\" id=\"percent\" class='vipTextBox' value=\"0\" width=\"80\" readonly=\"readonly\">％，<span>转让价每份<font style='color:red'>"+parseFloat(loanUnit).toFixed(1)+"</font>元</span>";
	content+="</div>";*/
	content+="<div style=\"line-height:25px;margin-top:5px\">";
	content+="    您投资的金额为 <font id=\"resultNum\" style=\"color:red\">"+parseInt(tenderMoney)+"</font>万元";
	content+="</div>";
	content+="<div style=\"line-height:25px;margin-top:5px\">";
	content+="    您决定转让 <font id=\"resultNum\" style=\"color:red\">"+parseInt(tenderMoney)+"</font>万元";
	content+="</div>";
	content+="<div style=\"line-height:25px;margin-top:5px\">";
	content+="    注:<font id=\"resultNum\" style=\"color:red\">项目一旦转让,由其它投资人认购，如截止时间无人认购，自动按原购入价由融资方或回购协议方回购。</font>";
	content+="</div>";
	...
	content+="        $(this).next().attr(\"readonly\",\"readonly\");";
	content+="        $(this).next().next().html(\"转让价每份<font style='color:red'>\"+parseFloat("+loanUnit+").toFixed(1)+\"</font>元\");";
	content+="    }else if ($(this).val()==\"1\"){";
	content+="        $(this).next().val(\"0\");";
	content+="        $(this).next().removeAttr(\"readonly\");";
	content+="        $(this).next().next().html(\"转让价每份<font style='color:red'>\"+parseFloat("+loanUnit+"*(1-$(this).next().val()/100)).toFixed(1)+\"</font>元\");";
	content+="    }else{";
	content+="        $(this).next().val(\"0\");";
	content+="        $(this).next().removeAttr(\"readonly\");";
	content+="        $(this).next().next().html(\"转让价每份<font style='color:red'>\"+parseFloat("+loanUnit+"*(1+$(this).next().val()/100)).toFixed(1)+\"</font>元\");";
	...
	content+="    	$(this).next().text(parseFloat($(this).val()*"+loanUnit+").toFixed(1));";
	content+="	  }";
	content+="});";
	...
	content+="        $(\"#resultPrice\").text(parseFloat("+loanUnit+"*(1-$(this).val()/100)).toFixed(1));";
	content+="        $(this).next().find(\"font\").text(parseFloat("+loanUnit+"*(1-$(this).val()/100)).toFixed(1));";
	content+="    }else{";
	content+="        $(\"#resultPrice\").text(parseFloat("+loanUnit+"*(1+$(this).val()/100)).toFixed(1));";
	content+="        $(this).next().find(\"font\").text(parseFloat("+loanUnit+"*(1+$(this).val()/100)).toFixed(1));";
	content+="    }}";
...
function select(no) {
		...
		window.location.href = "/shop/investManage.htm?no=" + no;
